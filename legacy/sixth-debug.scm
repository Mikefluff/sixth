      ...
      (cond ((null? rest) (error "Missing ;"))
            ((and (string? (car rest)) (string=? (car rest) ";"))
             (let ((def-str (string-join (reverse (map (lambda (x) (if (number? x) (number->string x) x)) body)) " ")))
               (display "DEBUG: Defining ") (display name) (display " as: ") (display def-str) (newline)
...
(define (load-file filename)
  (display "DEBUG: Loading file: ") (display filename) (newline)
  (when (file-exists? filename)
    (call-with-input-file filename
      (lambda (port)
        (let ((content (read-all-chars port)))
          (display "DEBUG: File content: ") (display content) (newline)
          (eval-string content))))))
